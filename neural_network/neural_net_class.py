...
class NeuralNet:
    ...
    def obtain_deltas(self):
        ...
        for l in range(L):
            if (l < L-1):
                ...
                delta[l] = np.multiply(current_layer.activation_function(current_layer.a), suma)
    ...
    def gradient_descent(self, learning_rate, nIter, data_train, label_train, data_test, label_test):
        cont = 0
        self.feed_forward(data_train)
        while (cont < nIter):
            cont += 1
    ...
    def obtain_grad(self):
        L = self.L
        for l in range(L):
            grad = self.layers[l].grad
            delta = self.layers[l].delta
            if (l > 0):
                grad = self.X[l-1].T.dot(delta)
            else:
                grad = data_train.T.dot(delta)
...
class Layer:
    def __init__(self, dimensions, activation_function):
        ...
        self.weights = correction * np.rand((I, J))
        self.activation_function = activation_function
        self.grad = np.empty((I, J))
        self.delta = np.empty((M, J))
        
        
def main():
        L = 4
        layer_dimensions = L*[20]
        layer_dimensions[0] = M
        ...
        NeuralNetwork = NeuralNet(4, layer_dimensions, activation_functions, derivation_functions)
        NeuralNetwork.gradient_descent(0.1, 4000, data_train, data_test)
